...
async function resolveCustomerId(
  ...
  if (!customer && userEmail) {
    const { data: unlinked } = await admin
      ...
      .maybeSingle();
    if (unlinked) {
      await admin.from("customers").update({ auth_user_id: userId }).eq("id", unlinked.id).eq("tenant_id", tenantId);
      customer = unlinked;
    }
  }
...
export async function getCustomerTickets(
  ...
  const admin = createAdminClient();
  const customerId = await resolveCustomerId(tenantId, userId, userEmail);
